...
def test_enhance_journal(tmp_path):
    # Test enhancing a journal with AI-generated content
...
Another beautiful day...""")
    ...
    def mock_claude_response(metadata):
        return f"Trail Context: The section from {metadata['start_location']} to {metadata['destination']} typically features..."
    
    # Enhance the journal
    enhance_journal(input_file, output_file, claude_client=mock_claude_response)
...
def test_enhance_journal_handles_api_errors(tmp_path):
...
This was a great day on the trail...""")
    ...
    def mock_failing_claude(metadata):
        raise Exception("API Error")
    
    # Should not raise an exception
    enhance_journal(input_file, output_file, claude_client=mock_failing_claude)
    
    # Should still have the original content
    enhanced_content = output_file.read_text()
    assert "This was a great day on the trail" in enhanced_content
    assert "API Error" not in enhanced_content 